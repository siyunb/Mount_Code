...
####Defining pathways
toplvl=/mnt/d/wangsiyu/Data/BIDS_NKI_walkthrough
dcmdir=/mnt/d/wangsiyu/Data/BIDS_NKI_walkthrough/Dicom
...
for direcs in session1 session2
do
...
do
	...
	tempdiff=`echo $Diff | sed '1q;d'`
	tempdiffext=${tempdiff##*.}
	tempdifffile=${tempdiff%.*}
	Sessionnum=`echo $tempdifffile | cut -d '_' -f 2`
	Difflast=`echo ${Sessionnum: -1}`
	if [ $Difflast -eq 2 ]                  #更改session个数
    then 
	    ses=2
	else
	    ses=1
...
do 
    ...
    Difflast=`echo "${Sessionnum: -1}"`
    if [[ $Modality = "dwi" && $Difflast -eq 2 ]]
    then
	    mv ${Orgfile}.${Orgext} ses-2/dwi
    else
        if [[ $Modality = "dwi" && $Difflast -eq 1 ]]
        then
...
###Convert dcm to nii
for direcs in TfMRI_breathHold_1400 TfMRI_eyeMovementCalibration_1400 TfMRI_eyeMovementCalibration_645 TfMRI_visualCheckerboard_1400 TfMRI_visualCheckerboard_645 session1 session2
do
    if [[ $direcs = "session1" || $direcs = "session2" ]]
    then
        for rest in RfMRI_mx_645 RfMRI_mx_1400 RfMRI_std_2500
        do 
...
for ((i=1;i<=${checkerfiles};i++))
do
    Checker=`ls *CHECKERBOARD*` #This is to refresh the Checker variable, same as the Anat case
    tempcheck=`echo $Checker | sed '1q;d'` #Capture new file to change
    tempcheckext=${tempcheck##*.}
    tempcheckfile=${tempcheck%.*}
    TR=`echo $tempcheck | cut -d '_' -f 4` #f4 is the fourth field delineated by _ to capture the acquisition TR from the filename
    ...
    if [ $tempcheckext = 'json' ]
    then
        sed -i '1a \\t"TaskName":"Checkerboard",' sub-${subj}_ses-1_task-Checkerboard_acq-TR${TR}_bold.${tempcheckext}
    else
        :
    fi
done
...
for ((i=1;i<=${eyefiles};i++))
do
    Eye=`ls *EYE*`
    tempeye=`echo $Eye | sed '1q;d'`
    tempeyeext=${tempeye##*.}
    tempeyefile=${tempeye%.*}
    TR=`echo $tempeye | cut -d '_' -f 4` #f4 is the fourth field delineated by _ to capture the acquisition TR from the filename
    ...
    if [ $tempeyeext = 'json' ]
    then
        sed -i '1a \\t"TaskName":"eyemovement",' sub-${subj}_ses-1_task-eyemovement_acq-TR${TR}_bold.${tempeyeext}
    else
        :
    fi
done
...
for ((i=1;i<=${breathfiles};i++))
do
    Breath=`ls *BREATH*`
    tempbreath=`echo $Breath | sed '1q;d'`
    ...
    if [ $tempbreathext = "json" ]
    then
        sed -i '1a \\t"TaskName":"breathhold",' sub-${subj}_ses-1_task-breathhold_acq-TR${TR}_bold.${tempbreathext}
    else
        :
    fi
done
...
for TR in 645 1400 CAP
do 
    for corrun in $(ls *REST_${TR}*) 
    do
        corrunfile=${corrun%.*}
        ...
        if [ $corrunfileext = 'json' ]
        then
            sed -i '1a \\t"TaskName":"rest",' sub-${subj}_ses-${ses}_task-rest_acq-TR${TR}_bold.${corrunfileext}
        else
            :
        fi
    done
...
do 
    ...
    Sessionnum=`echo $Orgfile | cut -d '_' -f2`
    Difflast=`echo "${Sessionnum: -1}"`
    if [[ $Modality = "bold" && $Difflast -eq 2 ]]
    then
	    mv ${Orgfile}.${Orgext} ses-2/func
    else
    if [[ $Modality = "bold" && $Difflast -eq 1 ]] 
    then
...
if [ -e ${niidir}/task-Checkerboard_acq-TR645_events.tsv ]
then
	:
else
...
if [ -e ${niidir}/task-Checkerboard_acq-TR1400_events.tsv ]
then
	:
else
...
if [ -e ${niidir}/task-eyemovement_acq-TR645_events.tsv ]
then
	:
else
#Create events file with headers
...
onlength=`cat /mnt/c/Users/kumad/Desktop/EyemovementCalibParadigm.txt | wc -l`
for ((i=2;i<=$((onlength-1));i++)) 
do 
    ontime=`cat /mnt/c/Users/kumad/Desktop/EyemovementCalibParadigm.txt | sed "${i}q;d" | cut -d ',' -f 1`
...
if [ -e ${niidir}/task-eyemovement_acq-TR1400_events.tsv ]
then
	:
else
...
if [ -e ${niidir}/task-breathhold_acq-TR1400_events.tsv ] 
then
	:
else
#Create events file with headers
